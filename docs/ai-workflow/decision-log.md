...
## 2026-05-06 - 피드백 보완 문장을 +1 개선 방식으로 변경

### 변경 전

최종 피드백은 사용자가 말한 문장을 바탕으로 자연스럽고 완성도 높은 문장을 제공했다.

```text
I want ice latte → Can I get an iced latte, please?
I want small latte → Can I get a small latte, please?
```

### 변경 후

보완 문장은 완벽한 정답보다 기존 문장에서 한 단계만 개선한 문장으로 제공한다.

```text
I want ice latte → I want an iced latte.
small size → Small, please.
here → For here, please.
```

또한 각 발화마다 다음 정보를 함께 제공한다.

```text
외국인 이해도
보완 문장 사용 시 예상 상승폭
보완 문장을 사용했을 때의 예상 이해도
```

### 변경 이유

Say Now의 핵심 가치는 “내 영어가 실제로 통하는지 확인하고, 바로 한 단계만 더 낫게 말해보는 것”이다. 초보자에게 너무 완성된 문장을 바로 제시하면 사용자가 본인의 표현과 피드백 사이의 거리를 크게 느낄 수 있다.

따라서 피드백은 “완벽한 영어”보다 “내가 방금 한 말에서 조금만 고친 영어”가 제품 경험에 더 맞다.

### 성능 영향

발화별 이해도와 상승폭은 서버 규칙으로 계산한다. 추가 LLM 호출이 없으므로 응답 시간 증가는 사실상 없다.

### 비용 영향

로컬 Ollama 실행 기준 비용 변화는 없다. 외부 API로 전환해도 추가 호출이 없기 때문에 API 비용이 늘지 않는다.

### 리소스 영향

점수 계산은 slot 개수, transcript 패턴, 짧은 응답 여부를 보는 단순 문자열 처리다. CPU/메모리 영향은 무시할 수 있는 수준이다.

### 구현 복잡도 영향

발화별 점수 계산과 보완 문장 상승폭 규칙이 추가됐다. 대신 프론트에서 사용자가 “현재 표현이 얼마나 통하고, 보완하면 얼마나 더 좋아지는지”를 바로 이해할 수 있다.
